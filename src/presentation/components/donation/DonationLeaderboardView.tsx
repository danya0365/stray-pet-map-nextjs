...
const TABS = [
  { id: "weekly" as const, label: "ฮีโร่ประจำสัปดาห์", icon: Sparkles },
  { id: "alltime" as const, label: "ฮอลล์ออฟเฟม", icon: Crown },
];
...
interface Props {
  initialViewModel: LeaderboardViewModel | null;
}

export function DonationLeaderboardView({ initialViewModel }: Props) {
  const [presenterState, presenterActions] = useDonationPresenter();

  const [activeTab, setActiveTab] = useState<"weekly" | "alltime">("weekly");
  const [entries, setEntries] = useState<DonationLeaderboardEntry[]>(
    initialViewModel?.entries ?? [],
  );
  const [stats, setStats] = useState<DonationStats | null>(
    initialViewModel?.stats ?? null,
  );
  const [isLoading, setIsLoading] = useState(false);

  const fetchData = useCallback(
    async (tab: "weekly" | "alltime") => {
      setIsLoading(true);
      try {
        await presenterActions.fetchLeaderboard(tab);
        await presenterActions.fetchStats();
        setEntries(presenterState.leaderboard);
        setStats(presenterState.stats);
      } finally {
        setIsLoading(false);
      }
    },
    [presenterActions, presenterState.leaderboard, presenterState.stats],
  );
  ...
  return (
    <div className="min-h-screen bg-background">
      {/* Hero */}
      <div className="bg-linear-to-br from-primary/10 via-secondary/5 to-background px-4 py-12">
